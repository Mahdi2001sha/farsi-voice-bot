...
# PATHS - Create in parent directory (Farsi to Text\)
...
LOG_HEADERS = ["id", "user_id", "timestamp", "audio_filename", "language_requested",
               ...
               "text_processed", "confidence", "reliability", "detected_commands"]
...
async def transcribe_file(
    ...
    if not quality_assessment["valid"]:
        raise HTTPException(400, f"Audio validation failed: {quality_assessment['issues']}")
    ...
    text_processed, _ = process_transcription(text)
    confidence_data = calculate_confidence(segments, text)
    keywords = extract_keywords(text_processed)

    processing_ms = int((time.time() - start) * 1000)
    log_id = _write_log(
        user_id, audio_filename, language, detected_lang, duration, processing_ms,
        text, text_processed, confidence_data, keywords
    )

    print(f"[{log_id}] Processed - Confidence: {confidence_data['confidence']}%")
...
async def transcribe_base64(req: TranscribeBase64Request):
    ...
    timestamp_str = datetime.now().strftime("%Y%m%d-%H%M%S")
    audio_filename = f"{timestamp_str}-{req.user_id}.{req.audio_format}"
    saved_audio_path = AUDIO_DIR / audio_filename
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=f".{req.audio_format}") as tmp:
        tmp.write(audio_bytes)
        tmp_path = tmp.name

    try:
        text, detected_lang, duration, segments = _run_whisper(tmp_path, req.language or None)
    finally:
        os.unlink(tmp_path)
    ...
    processing_ms = int((time.time() - start) * 1000)
    log_id = _write_log(
        req.user_id, audio_filename, req.language, detected_lang, duration, processing_ms,
        text, text_processed, confidence_data, keywords
    )
...
@app.get("/logs")
def get_logs(limit: int = 50, user_id: str = None, min_confidence: float = 0):
    if not LOG_FILE.exists():
        return {"logs": []}
    ...
    if min_confidence > 0:
        rows = [r for r in rows if float(r.get("confidence", 0)) >= min_confidence]
